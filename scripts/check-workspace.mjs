...
// The checked-in browser registry is generated from mineral JSON. Fail before
// build/test if somebody edits the generated module instead of its source data.
const dataSource = new URL('../packages/crystal-data/src/', import.meta.url);
const mineralOrder = ['fluorite', 'quartz', 'calcite', 'pyrite', 'anatase', 'albite', 'gypsum', 'forsterite', 'beryl'];
...
for (const name of mineralOrder) {
    const json = readFileSync(new URL(`minerals/${name}.json`, dataSource), 'utf8').trim();
    generatedLines.push(`export const ${name.toUpperCase()} = defineMineral(${json});`, '');
}
assert.equal(
    readFileSync(new URL('records.ts', dataSource), 'utf8'),
    generatedLines.join('\n'),
    'crystal-data records.ts is stale; edit mineral JSON and run node scripts/generate-mineral-records.mjs',
);
console.log('Generated mineral records are synchronized.');
