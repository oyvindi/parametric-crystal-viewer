...
Keep commit messages focused on the change. Do not add automated attribution text such as `Generated by ...` or `Co-Authored-By: ...` footers.

Add a file or pattern to `.gitignore` when it should not be tracked by Git.
