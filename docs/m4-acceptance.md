...
| Existing records use the permanent path | [Fluorite](../packages/crystal-data/src/minerals/fluorite.json) and [quartz](../packages/crystal-data/src/minerals/quartz.json) are generated through `defineMineral`. Their revisions are `m4-1`; scientific values, form settings, and habit IDs are unchanged. Provenance coverage now explicitly includes curated form selections/settings and quartz variants. |
