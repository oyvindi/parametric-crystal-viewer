...
export function generateCrystalGeometry(input: MorphologyInput): GeometryResult {
    ...
    const planes = [] as { id: string; normal: readonly [number, number, number]; distance: number }[];
    for (const form of active) {
        if (!Number.isFinite(form.development) || form.development > 1) return invalid("core.input.invalid-development", "Form development must be finite and within [0, 1].");
        ...
        for (const direction of directions.value) planes.push({ id: `${form.id}:${direction.indices.h},${direction.indices.k},${direction.indices.l}`, normal: direction.normal, distance: 1 / form.development });
    }
